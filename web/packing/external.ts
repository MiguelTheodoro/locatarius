...
export const External = (fallback: RequestInit & { url: string }) => {
    ...
    return <Argument extends any[], Return>(fn: (...args: Argument) => Partial<Record<"param" | "query" | "body", Record<string, any>>>, custom: RequestInit & { endpoint?: string }) => {
        ...
        return async <Response>(...args: Argument): Promise<{ code: number, data: Response }> =>  {
            ...
            //console.log("(HighOrderFunction.External) variable (fallback) -> ", fallback)
            //console.log("(HighOrderFunction.External) variable (custom) -> ", custom)
            //console.log("(HighOrderFunction.External) variable (body) -> ", JSON.stringify(fallback.body))
            
            
            const response = await fetch(url.toString(), { ...fallback, body: JSON.stringify(fallback.body), ...custom, headers: { ...fallback.headers, ...custom.headers  }, cache: "no-store"} as unknown as RequestInit)
            ...
            //console.log("(HighOrderFunction.External) variable (json) -> ", { json })
